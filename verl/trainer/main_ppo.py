...
class TaskRunner:
    def run(self, config):
        ...
        # Handle model loading based on load_method
        model_load_method = config.actor_rollout_ref.model.get("load_method", "huggingface")
        model_path = config.actor_rollout_ref.model.path
        
        if model_load_method == "huggingface":
            # For HuggingFace models, use the path directly (transformers will handle downloading)
            local_path = model_path
            print(f"Loading model from HuggingFace: {model_path}")
        elif model_load_method == "local":
            # For local models, use copy_to_local to handle HDFS paths if needed
            local_path = copy_to_local(model_path, use_shm=config.actor_rollout_ref.model.get("use_shm", False))
            print(f"Loading model from local path: {local_path}")
        else:
            raise ValueError(f"Invalid model_load_method: {model_load_method}. Must be 'huggingface' or 'local'")
