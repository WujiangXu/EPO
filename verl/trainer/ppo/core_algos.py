...
def compute_policy_loss_with_entropy_mask(
    old_log_prob,
    log_prob,
    advantages,
    response_mask,
    ...
    """
    Compute the clipped policy objective and related metrics for PPO.
    This function is used to compute the policy loss with entropy mask. To remove the policy loss with entropy in high rate of change.
    ...
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1) * entropy_mask
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
